...
test('Escape and closing the Library cancel placement, including deferred image dimensions', { timeout: 45000 }, async () => {
  ...
  try {
    for (const width of [1440, 700]) {
      ...
      assert.equal(await snapshot(), original, 'pointer release waits for the source decode despite known preview dimensions');
      await page.getByRole('button', { name: 'Library', exact: true }).click();
      await page.waitForTimeout(300);
      await page.evaluate(() => window.__releasePlacementDimensions());
      await page.waitForTimeout(100);
      assert.equal(await snapshot(), original, 'closing the Library cancels a dropped image still awaiting its source');
      await page.getByRole('button', { name: 'Library', exact: true }).click();
