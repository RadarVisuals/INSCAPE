...
test('Text module authors, reloads, renders and contains content at wide/narrow widths', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    await Promise.race([mount(), new Promise((_, reject) => setTimeout(() => reject(Error('Text review module imports timed out')), 30_000))]);
    ...
    await content.fill('The landscape began with a single shape.');
    await page.keyboard.press('Control+A'); await text.getByRole('button', { name: 'Bold', exact: true }).click();
    assert.equal(await content.locator('strong').innerText(), 'The landscape began with a single shape.');
    await text.getByRole('button', { name: 'Read', exact: true }).click();
    assert.equal(await text.locator('article strong').innerText(), 'The landscape began with a single shape.');
    assert.equal(await text.locator('.text-status').count(), 0, 'saved status takes no layout space');
    assert.equal(await text.locator('.system-workflow__detached-window-titlebar > strong').count(), 0, 'no duplicate window title');
    ...
    await text.getByRole('button', { name: 'Read', exact: true }).click();
    await text.getByRole('button', { name: 'Text settings', exact: true }).click();
    ...
    for (const [name, viewport] of [['wide', { width: 1440, height: 1000 }], ['narrow', { width: 390, height: 844 }]]) {
      ...
      const box = await text.locator('.system-workflow__instrument-window').boundingBox();
      ...
      assert.equal(await text.getByRole('textbox', { name: 'Article title', exact: true }).evaluate(el => el.scrollWidth <= el.clientWidth + 1), true, 'title wraps without clipping');
    }
    ...
    await visitor.getByText('The landscape began with a single shape.', { exact: true }).waitFor();
