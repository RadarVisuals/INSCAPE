...
test('independent Image: Library drop, free size, dock crop, wraparound flip, Lift, persistence and Visitor parity', { timeout: 120000 }, async () => {
  ...
  try {
    ...
    const cropImage = canvas.locator('img');
    const beforePan = await cropImage.getAttribute('style');
    ...
    assert.notEqual(await cropImage.getAttribute('style'), beforePan, 'dragging pans media inside the crop');
    ...
    assert.deepEqual(await page.evaluate(() => window.readDraft()), cropped);
    await grip.focus(); await dock.getByRole('checkbox', { name: 'Include Image in publication' }).check();
