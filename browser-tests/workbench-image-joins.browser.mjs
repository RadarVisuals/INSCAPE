...
test('Image joins survive Workbench capture, repeated grabs, pan and continuous zoom', { timeout: 180000 }, async () => {
    ...
    for (const density of [1, 1.25, 2]) {
      ...
      await page.evaluate(async () => {
        ...
        function Fixture(){
          ...
          window.readPositions=()=>positions.map((p,i)=>({left:p.left+(view.transforms[`image:${i}`]?.x||0),top:p.top+(view.transforms[`image:${i}`]?.y||0)}));
          window.camera=(zoom,x,y)=>{view.setScale(zoom);pan.setOffset({x,y});};
          window.configure=zoom=>{view.setScale(zoom);view.setTransforms({});setPositions([{left:100.3,top:100.7},{left:110.3,top:510.7}]);};
      ...
      for(const zoom of [.25,.33,.67,.99,1,1.01,1.37,2]){
        ...
        if (zoom === .67 && density === 1) {
          await page.screenshot({path: '.browser-test-runtime/workbench-joins-wide.png'});
          await page.setViewportSize({width: 390, height: 844});
          await page.screenshot({path: '.browser-test-runtime/workbench-joins-narrow.png'});
