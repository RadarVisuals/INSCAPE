...
2026-08-24 Activity event-time correction checkpoint:

- the Activity repository already canonicalized indexer timestamps from either seconds or milliseconds to milliseconds; the owner Activity formatter multiplied that canonical value by 1,000 a second time, producing years such as `58608`. The formatter now consumes the canonical millisecond instant directly;
- the displayed date and time remain the indexed on-chain event instant, not the time at which Activity is viewed or marked read. Missing, zero, malformed, or non-finite event times remain explicitly `UNKNOWN` instead of receiving an invented fallback date;
- the affected facts remain native LYX activity, LSP7/LSP8 transfer activity, and LSP26 follow/unfollow activity. Official [LSP7](https://docs.lukso.tech/standards/tokens/LSP7-Digital-Asset/), [LSP8](https://docs.lukso.tech/standards/tokens/LSP8-Identifiable-Digital-Asset/), and [LSP26](https://docs.lukso.tech/standards/accounts/lsp26-follower-system/) specifications were rechecked on 2026-08-24; their transfer, sender/recipient, follower/following, and notification semantics are unchanged;
- focused Activity, signal-domain, LUKSO-repository, controller, and browser coverage passed `14/14`; production-budget and owner-isolation coverage passed `18/18`; the production build transformed 5,431 modules and measured owner JavaScript at `370_971` raw / `112_076` gzip; the alternate-outDir matrix measured `370_971` / `112_056`; only `ownerJavaScript` was rebaselined to `370_971` / `112_079`, retaining 0 raw / 3 gzip bytes above the highest measured boundary;
- `build:check` passed at `802_502` initial JavaScript bytes and the emitted owner graph retained `leaks: []`. Wallet authority, uploads, signatures, transactions, ERC725Y writes, merges, production publication, and Tasks 6-7 remain unchanged and locked.

2026-08-24 verified on-chain LSP4 JSON/SVG checkpoint:

- Burnt Pix token `0x3983151e0442906000dab83c8b1cf3f2d2535f82` / `0x00000000000000000000000085bc3f6772107468dd9edf194f114b0c8c66eb71` proved the missing-media root cause: its direct `LSP4MetadataForTokenId` VerifiableURI resolves to a `data:application/json` document and its verified image resolves to a `data:image/svg+xml;base64` source, while the canonical resolver and published-media boundary rejected every `data:` URL before either LSP4 hash could be honored;
- the direct live metadata URI measured approximately `256_784` characters, the nested SVG URI `256_310` characters, and the decoded SVG `192_212` bytes. Both supported LSP2/LSP4 Keccak verification forms were checked against their declared hashes before use. The metadata declares `768 x 768`, the SVG declares `viewBox="0 0 1025 1025"`, and real Edge decoding is authoritative at `naturalWidth=150`, `naturalHeight=150`: every boundary therefore retains the same square aspect ratio without inventing dimensions;
- one bounded shared parser now admits only hash-authentic UTF-8 on-chain LSP4 JSON and hash-authentic, canonical-base64, inert SVG. It rejects oversize input, malformed UTF-8/base64, DOCTYPE/entities, script and embedded active content, event handlers, external or executable references, CSS imports, and external CSS URLs. HTTP/IPFS behavior remains unchanged; there is no Burnt Pix special case, NFT-only datatype, CSS geometry correction, wallet behavior, or relationship inference;
- the exact live token now resolves through direct RPC into Library and keeps the identical SVG source through drag/placement, owner Preview, technical metadata, Profile Document v9, and Visitor. A representative v9 document measured `258_476` canonical UTF-8 bytes and Visitor decoded the same source at `150 x 150`. Two representative placements remain below the existing `524_288`-byte document ceiling; three are rejected fail-closed by the unchanged v9 size gate, which remains the explicit risk for repeated very large inline works;
- focused Library/Creations/on-chain-media coverage passed `41/41`; focused v9/publication/geometry coverage passed `58/58`; the final representative owner Preview/Visitor check passed `7/7`; production-budget and owner-isolation coverage passed `18/18`; LUKSO standards passed `5/5`. The production build transformed 5,432 modules; primary owner JavaScript measured `373_708` raw / `113_395` gzip and the alternate output measured `373_708` / `113_391`. Only `ownerJavaScript` was rebaselined to `373_708` / `113_398`, retaining 0 raw / 3 gzip bytes above the highest measured boundary;
- `build:check` passed at `802_568` initial JavaScript bytes and the emitted owner graph retained `leaks: []`. The general Rollup chunk warning is unchanged. No upload, signature, transaction, ERC725Y write, deployment, merge, commit, push, production publication, Tasks 6-7 work, or Netlify configuration change occurred; production remains locked. Official [LSP4 Digital Asset Metadata](https://docs.lukso.tech/standards/tokens/LSP4-Digital-Asset-Metadata/) guidance was rechecked on 2026-08-24.

2026-08-24 compact hash-pinned on-chain media publication checkpoint:

- the 512 KiB failure was caused by Profile Document v9 serializing the complete approximately 256 KiB inline SVG in every placement. Raising the document ceiling, copying the work to IPFS, or weakening SVG verification was rejected. On-chain LSP4 media now publishes a compact generic ERC725Y content reference containing scope, canonical data key, and the exact metadata verification method/hash; ordinary HTTPS/IPFS and existing direct v9 media remain byte-shape compatible;
- Library direct RPC retains this compact reference only after the original LSP4 metadata VerifiableURI and selected nested SVG have already passed the bounded authentic-content path. Profile Document v9 then omits the expanded SVG (`media.url: null`) and Visitor performs a read-only `getDataForTokenId`, requires the returned pointer to match the pinned verification exactly, verifies and parses the bounded on-chain JSON, verifies the nested inert SVG, and only then exposes it to the existing browser decode/render path. Shared promise caching prevents repeated placements from repeating the RPC read; stale component results are ignored after unmount;
- the exact live Burnt Pix token `0x3983151e0442906000dab83c8b1cf3f2d2535f82` / `0x00000000000000000000000085bc3f6772107468dd9edf194f114b0c8c66eb71` resolved read-only from compact metadata hash `0xb02bc9f4744ae1686dcb30481fab6b41b2d11e6f499900381d8fe9a3d3c56e38` back to the authentic `256_310`-character SVG. Its declared `768 x 768` fallback remains square and the already proven real Edge decode remains canonical at `150 x 150`;
- focused Library, on-chain parser, Profile Document v9, publication, resolver, Visitor, and owner Preview coverage passed `96/96`; the regression includes 15 repeated and 15 distinct Burnt Pix-scale assets inside the unchanged 524,288-byte v9 ceiling, changed metadata hashes, changed nested content, malformed content, unknown dimensions, and browser-authoritative dimension handoff. LUKSO standards passed `5/5`;
- the production build transformed 5,433 modules and passed with initial JavaScript `802_568` raw, owner JavaScript `375_472` raw / `113_972` gzip in the primary output and `113_978` gzip in alternate output, owner CSS unchanged at `143_870` / `21_140`, and runtime `leaks: []`. Only the owner JavaScript ceiling was rebaselined to `375_472` / `113_981`; the build hygiene hook now runs after bundle files are written so fresh alternate output directories cannot race the generated manifest, and bounded Windows cleanup retries make that matrix deterministic. Production budget/runtime/isolation coverage passed `18/18`; the normal production build and `build:check` passed;

2026-08-24 alpha viewport-raster seam checkpoint:

- user validation confirmed that the remaining edge-to-edge hairlines varied with viewport size rather than canonical placement geometry. Shared placement boundaries were already pixel-snapped; independently transformed full-bleed media could still expose a raster seam at an exact clip edge;
- the renderer now adds a one-pixel raster bleed only where a media edge already equals its canonical opening edge. Placement, opening, crop, resize, rotate, mirror, snap, intentional contain letterboxing, and Profile Document v9 geometry remain unchanged, and Owner plus Visitor use the same projection;
- the primary production build measured owner JavaScript at `376_121` raw / `114_192` gzip, the alternate output measured `376_121` / `114_196`, and Netlify Linux measured `376_121` / `114_205`. Only `ownerJavaScript` was rebaselined to `376_121` / `114_208`, retaining 0 raw / 3 gzip bytes above the highest measured platform boundary; production publication remains locked.
- Visitor now requires read-only LUKSO RPC availability to reconstruct compact on-chain media. RPC unavailability, a changed pinned pointer, malformed JSON, unsafe SVG, or a changed nested hash remains visibly unavailable and never falls back to invented media. No NFT-only datatype, upload, signature, transaction, ERC725Y write, deployment, merge, commit, push, production publication, Tasks 6-7 work, or Netlify configuration change occurred; production remains locked.
