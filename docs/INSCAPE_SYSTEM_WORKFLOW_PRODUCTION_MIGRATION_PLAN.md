...
- the primary production build measured owner JavaScript at `376_121` raw / `114_192` gzip and the alternate output measured `376_121` / `114_196`. Only `ownerJavaScript` was rebaselined to `376_121` / `114_199`, retaining 0 raw / 3 gzip bytes above the highest measured boundary; production publication remains locked.
