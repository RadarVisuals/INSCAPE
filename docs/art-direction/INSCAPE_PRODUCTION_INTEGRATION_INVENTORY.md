...
Category rollback is code-only: stop injecting `categoryCommands`, remove the Browser menu/dialog activation and guarded store action, and restore read-only wording. The existing workspace-v8 format and category records remain valid; no data migration, category deletion, asset rewrite, canonical draft rewrite, or publication rollback is authorized. PORTALS, drag/drop, bulk organization, AI categorization, category artwork, publication/visitor navigation, wallet/IPFS activity, favorites editing, legacy routing/cleanup, and Phase 5B.6 presentation work remain excluded.
