...
Published-visitor browser-harness maintenance checkpoint (2026-08-03): `[x]` — browser-test infrastructure now exercises the selected production visitor dispatch honestly: validated Version 8 enters the canonical nine-table `VisitorLatticeWorld`, while Version 7 remains readable through the existing legacy boundary. Semantic fixture/runtime/table readiness replaces the obsolete `.published-home-world__header` presentation selector that reproduced as the post-mount bootstrap timeout. Stale profile-card geometry, Categories/detached-browser/flip-viewer, and Home-only drag expectations are retired and explicitly replaced by canonical projection, production viewer, Keeper pointer-follow, and lattice-navigation coverage; security, publication media, CSP, compatibility, Directory/Return, narrow accessibility, visitor storage isolation, and StrictMode/provider lifecycle coverage remain. Focused fixture/lifecycle passes 18/18, the affected visitor/publication matrix passes 150/150, the published visitor Edge harness passes 12/12 with zero failures/cancellations/skips and complete owned-process cleanup, the corrected direct/iframe fixture and browser matrix pass 1/1 each, and the full sequential suite passes 1,027/1,027. Production build, the 1,240,851-byte initial-JavaScript budget check, and `git diff --check` pass with no production-code change. The unchanged real-App owner-profile routing matrix still reproduces its historical owner A-to-B timing failure at 2/3, with zero cancellations and complete cleanup. Tasks 1–9 remain `[x]`; Task 10 and Phase 10 remain separately unauthorized.
