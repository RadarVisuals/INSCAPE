...
export const PROFILE_DOCUMENT_V9_MEDIA_STATUS = Object.freeze({
  READY: 'ready', RESOLVING: 'resolving', UNSUPPORTED: 'unsupported', UNAVAILABLE: 'unavailable', INVALID: 'invalid',
});
...
export function adaptProfileDocumentV9Media(asset, options) {
  ...
  const compactReference = asset.media.url === null && asset.media.reference;
  const src = resolvePublishedAssetUrl(compactReference ? options?.resolvedUrl : asset.media.url, options);
  if (compactReference && !options?.resolutionComplete) return Object.freeze({
    status: PROFILE_DOCUMENT_V9_MEDIA_STATUS.RESOLVING, label, stableAssetId: asset.stableAssetId,
  });
  if (!src) return Object.freeze({
    status: PROFILE_DOCUMENT_V9_MEDIA_STATUS.UNAVAILABLE, label, stableAssetId: asset.stableAssetId,
  });
  const decodedDimensions = options?.decodedDimensions;
  const dimensions = Number.isSafeInteger(decodedDimensions?.width) && decodedDimensions.width > 0
    && Number.isSafeInteger(decodedDimensions?.height) && decodedDimensions.height > 0
    ? Object.freeze({ width: decodedDimensions.width, height: decodedDimensions.height })
