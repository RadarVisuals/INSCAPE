...
export default function PublishedProfileDocumentSpaceWindow({ space, minimized = false, dragHandleProps = {}, onMinimize, onClose }) {
  ...
  return <article className="collection-window folder-window profile-document-space-window" data-published-document-only data-minimized={minimized || undefined}>
    <header className="collection-window__header published-space-window__drag-handle" {...dragHandleProps}><div><span>Published space / verified document</span><h2>{space.label}</h2></div><p>{projected.length} assets</p><div className="published-space-window__controls"><button type="button" onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.currentTarget.focus(); onMinimize?.(); }} aria-label={`${minimized ? 'Restore' : 'Minimize'} ${space.label}`}><Minus aria-hidden="true" /></button><button type="button" onPointerDown={(event) => event.stopPropagation()} onClick={onClose} aria-label={`Close ${space.label}`}><X aria-hidden="true" /></button></div></header>
    {!minimized && <div className="collection-window__body folder-window__body"><main className="collection-content">
      ...
      <AssetGrid assets={projected} workspace={EMPTY_WORKSPACE} onSelect={setSelectedId} emptyMessage={`${space.label} is empty.`} renderImage={(props) => <PublishedImage {...props} />} />
    </main><AssetPreview asset={selected} workspace={EMPTY_WORKSPACE} onClose={() => setSelectedId(null)} renderImage={(props) => <PublishedImage {...props} />} /></div>}
  </article>;
}
