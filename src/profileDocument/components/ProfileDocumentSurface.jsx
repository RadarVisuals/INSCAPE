...
export default function ProfileDocumentSurface({ document, heading, onExit, SpaceWindow }) {
  ...
  return <main className="public-shell profile-document-preview" data-preview-mode="visitor" data-interface-visible aria-label="Profile document visitor preview">
    <header className="public-shell__masthead profile-document-preview__header"><div className="system-hud__identity">
      <h1>[ <span className="system-hud__brand-accent">{heading}</span> ]</h1>
    ...
    </div>{onExit && <nav className="system-hud__commands"><button type="button" onClick={onExit}>[ Exit Preview ]</button></nav>}</header>
    ...
    <section className="profile-document-preview__objects" aria-label="Published canvas artwork">
      {[...document.canvasObjects].sort((a,b)=>a.order-b.order).map((object)=>{const asset=projectDocumentAsset(object.asset);return <FramedArtwork key={object.id} object={{...object,stableAssetId:object.asset.stableAssetId,visitorVisible:true,presentationOrder:object.order}} asset={asset} arranging={false} selected={false} compact={false} onActivate={()=>setOpenArtworkId(object.id)} style={{left:`${object.placement.column/24*100}%`,top:`${object.placement.row/13*100}%`,width:`${object.span.columns/24*100}%`,height:`${object.span.rows/13*100}%`,zIndex:10+object.order}} />;})}
    </section>
    {openSpace && <section className="module-shell module-shell--expanded module-shell--collection module-shell--folder profile-document-preview__window"><SpaceWindow space={openSpace} onClose={() => setOpenSpaceId(null)} /></section>}
    {openArtwork && (()=>{const asset=projectDocumentAsset(openArtwork.asset);return <section ref={artworkDialogRef} className="profile-document-preview__artwork" role="dialog" aria-modal="true" aria-label={`Artwork preview: ${asset.name}`}><header><strong>{asset.name}</strong><button className="profile-document-preview__artwork-close" type="button" onClick={()=>setOpenArtworkId(null)} aria-label="Close artwork preview">×</button></header>{asset.imageUrl?<img src={asset.imageUrl} alt={asset.name}/>:<p>Artwork unavailable</p>}</section>;})()}
  </main>;
}
