...
import { cleanProfileDocumentIdentity } from './profileDocumentIdentity.js';
import { createIdentityRackPresentation } from './profileDocumentRacks.js';
...
export function buildProfileDocumentV3({ profileAddress, workspace, assets = [], publicPresentation, signalSettings, profileIdentity,
  modulePositions = {}, systemPresentation = {}, rackPresentation = {}, documentId, revision = 1, createdAt, exportedAt }) {
  ...
  const identitySystemModule = { id: 'identity', visible: true, placement: cleanPosition(modulePositions.identity), startOpen: systemPresentation.identity?.startOpen === true, windowGeometry: cleanWindowGeometry(systemPresentation.identity?.windowGeometry) };
  const racks = [createIdentityRackPresentation(rackPresentation.identity, identitySystemModule)];
  return {
    ...
    network: { ...PROFILE_DOCUMENT_NETWORK }, profile: { address, cachedIdentity: cleanProfileDocumentIdentity(profileIdentity, address, racks) },
    presentation: { keeperId: publicPresentation?.keeperId || 'abyssal_eye', stageId: publicPresentation?.stageId || 'moonpurple',
      ...
      systemModules: [identitySystemModule, { id: 'signals', visible: true, placement: cleanPosition(modulePositions.signals), startOpen: systemPresentation.signals?.startOpen === true, windowGeometry: cleanWindowGeometry(systemPresentation.signals?.windowGeometry) }],
      racks,
...
export const buildProfileDocumentV5 = buildProfileDocumentV3;
