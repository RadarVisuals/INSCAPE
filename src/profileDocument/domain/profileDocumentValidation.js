...
export function validateProfileDocument(input, { rawSize } = {}) {
  ...
  if (!exactKeys(presentation, ['keeperId', 'stageId', 'avatarShape', 'environment', 'systemModules', 'signals']) || !KNOWN_KEEPER_IDS.includes(presentation?.keeperId) || !KNOWN_STAGE_IDS.includes(presentation?.stageId) || (presentation?.avatarShape !== undefined && !['square', 'round'].includes(presentation.avatarShape))) fail('presentation', 'invalid_presentation', 'Unknown Keeper, stage, avatar shape, or presentation fields');
