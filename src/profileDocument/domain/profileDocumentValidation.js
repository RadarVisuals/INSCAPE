...
export function validateProfileDocument(input, { rawSize } = {}) {
  ...
  if (!exactKeys(presentation, ['keeperId', 'stageId', 'avatarShape', 'visitorNavigation', 'environment', 'systemModules', 'signals']) || !KNOWN_KEEPER_IDS.includes(presentation?.keeperId) || !KNOWN_STAGE_IDS.includes(presentation?.stageId) || (presentation?.avatarShape !== undefined && !['square', 'round'].includes(presentation.avatarShape))) fail('presentation', 'invalid_presentation', 'Unknown Keeper, stage, avatar shape, or presentation fields');
  if (!exactKeys(presentation?.visitorNavigation, ['showCategories', 'showCreations']) || typeof presentation?.visitorNavigation?.showCategories !== 'boolean' || typeof presentation?.visitorNavigation?.showCreations !== 'boolean') fail('presentation.visitorNavigation', 'invalid_visitor_navigation', 'Invalid visitor navigation visibility');
...
export function parseProfileDocumentJson(raw) {
  ...
  if ([1, 2, 3, 4, 5, 6].includes(input?.version)) return migrateProfileDocument(input);
