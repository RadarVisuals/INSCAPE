...
test('identity rack publication is privacy-aware and opt-in per public module', () => {
  ...
  assert.notEqual(
    profileDocumentContentFingerprint(privateByDefault),
    profileDocumentContentFingerprint(publicIdentity),
    'changing public identity disclosure must stale the previous snapshot'
  );
});
...
test('draft preview clones the current public projection without replacing the publication snapshot', () => {
  const snapshot = build({ documentId: 'profile:snapshot', revision: 1 });
  const draft = build({ documentId: 'profile:draft', revision: 2, profileIdentity: { name: 'Current draft' } });
  ...
  assert.equal(state.snapshot.documentId, 'profile:snapshot');
