export default function CollectionSidebar({ workspace, activeView, onOpenView, onCreate, onRename, onDelete, editMode = false }) {
  ...
  return (
    <aside className="collection-sidebar" aria-label="Collection views">
      ...
      {workspace.folders.map((folder) => (
        <div className="collection-folder" key={folder.id} data-active={activeView.id === folder.id || undefined} data-edit-mode={editMode || undefined}>
          ...
          {editMode && <button type="button" title="Rename folder" onClick={() => onRename(folder)} aria-label={`Rename ${folder.name}`}>R</button>}
          {editMode && <button type="button" onClick={() => askDelete(folder)} aria-label={`Delete ${folder.name}`}>×</button>}
        </div>
      ))}
      <button className="collection-sidebar__new" type="button" onClick={onCreate}>+ New folder</button>
    </aside>
  );
}
