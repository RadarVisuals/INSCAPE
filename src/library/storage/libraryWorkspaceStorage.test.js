...
test('Phase 1 data migrates to v3 without changing favorites, folder names, or multi-folder memberships', () => {
...
test('pinned launcher identity and placement persist across a reload', () => {
  ...
  assert.deepEqual(loadLibraryWorkspace(storage, profile).canvas.launchers, [{
    id: 'library:folder:archive', viewType: 'folder', folderId: 'archive', visitorVisible: false, position: { column: 3, row: 4 }, windowPosition: { column: 1, row: 2 }
  }]);
});
