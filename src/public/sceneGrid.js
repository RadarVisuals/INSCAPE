...
export function findScenePlacementAtPointer({ id, pointer, gridClientRect, zoom = 1, span, items, geometry }) { const scale=Math.max(0.01,Number(zoom)||1);const requested={column:Math.floor((pointer.x-gridClientRect.left)/(geometry.cellWidth*scale)),row:Math.floor((pointer.y-gridClientRect.top)/(geometry.cellHeight*scale))};return findScenePlacement(id,requested,span,items,geometry); }
