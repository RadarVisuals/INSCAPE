...
import { Bell, Check, ChevronDown, ChevronUp, ChevronsDown, ChevronsUp, Compass, Copy, Crop, Eye, FlipHorizontal2, FlipVertical2, Folder, Frame, Grid3X3, History, Library, Plus, RotateCw, Search, Settings2, Trash2, Upload, UserCheck, UserRound, UsersRound, X } from 'lucide-react';
import {
...
import OwnerShellIdentityDossier from './OwnerShellIdentityDossier.jsx';
import './ownerShellPrototype.css';
...
function OwnerShellFocusArtwork({ entry, phase }) {
  ...
  return <div className="owner-shell-study__focus-artwork" ref={rootRef}>
...
function PrototypeSelectMenu({ label, menuSurface, onChange, options, value }) {
  ...
  return <>
    <button aria-expanded={open} aria-haspopup="listbox" className="owner-shell-study__rail-select" onClick={() => open ? close(false) : show()} onKeyDown={(event) => {
      if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp') return;
      event.preventDefault();
      show(event.key === 'ArrowDown' ? selectedIndex : (selectedIndex - 1 + options.length) % options.length);
    }} ref={triggerRef} type="button">
      <span>{label}</span><ChevronUp aria-hidden="true" size={12} />
    </button>
    {open && position && createPortal(<div className="owner-shell-study__select-popover" data-lattice-menu-surface data-menu-surface={menuSurface}
...
function OwnerShellWorkspaceRail({ clearFilters, hasActiveFilters, labelsVisible, menuSurface, onLabelsVisibleChange,
  ...
  return <div className="owner-shell-study__workspace-rail-controls">
    <label className="owner-shell-study__workspace-search">
      <Search aria-hidden="true" size={13} /><span>SEARCH</span>
      <input aria-label="Search" onChange={(event) => onQueryChange(event.target.value)} type="search" value={query} />
    </label>
    <label className="owner-shell-study__workspace-size">
    ...
    <label className="owner-shell-study__workspace-labels"><input checked={labelsVisible}
      onChange={(event) => onLabelsVisibleChange(event.target.checked)} type="checkbox" /><span>LABELS</span></label>
    {hasActiveFilters && <button aria-label="Clear filters" className="owner-shell-study__workspace-clear" onClick={clearFilters} title="Clear filters" type="button"><X size={13} /></button>}
  </div>;
}
...
export default function OwnerShellPrototype() {
  ...
  const [discoverSidebarWidth, setDiscoverSidebarWidth] = useState(174);
  ...
  const discoverSidebarResizeRef = useRef(null);
  ...
  }, [activityHistoryOpen, activityOpen, discoverOpen, identityDossierSession, profileOpen, settingsOpen, tableMapOpen]);
  ...
  const activeTableIndex = Math.max(0, tables.findIndex(({ id }) => id === activeTableId));
  ...
  const openIdentityDossier = () => {
    const source = profileIdentityRef.current;
    if (!source) return;
  ...
  const visibleActivityHistory = ACTIVITY.filter((event) => {
    const matchesFilter = activityHistoryFilter === 'ALL'
      || (activityHistoryFilter === 'UNREAD' ? event.unread : event.type === activityHistoryFilter);
  ...
  const beginDiscoverSidebarResize = (event) => {
    if (event.button !== 0) return;
    event.preventDefault();
    discoverSidebarResizeRef.current = {
      pointerId: event.pointerId,
      startWidth: discoverSidebarWidth,
      startX: event.clientX,
    };
    event.currentTarget.setPointerCapture?.(event.pointerId);
  };
  const updateDiscoverSidebarResize = (event) => {
    const gesture = discoverSidebarResizeRef.current;
    if (!gesture || gesture.pointerId !== event.pointerId) return;
    const width = Math.min(300, Math.max(48, gesture.startWidth + event.clientX - gesture.startX));
    setDiscoverSidebarWidth(width);
  };
  const finishDiscoverSidebarResize = (event) => {
    if (discoverSidebarResizeRef.current?.pointerId !== event.pointerId) return;
    discoverSidebarResizeRef.current = null;
  ...
  return <main className="owner-shell-study" data-canvas-context={canvasContext} data-lattice-menu-surface data-menu-surface={menuSurface} data-preview={preview || undefined} data-surface={gridSurface}
    style={Object.fromEntries(Object.entries(dockAnchors).map(([key, left]) => [`--dock-${key}-left`, `${left}px`]))}>
    <header className="owner-shell-study__global">
      <div className="owner-shell-study__identity">
      ...
      <button aria-expanded={tableMapOpen} className="owner-shell-study__table" disabled={preview} onClick={() => toggleDockPanel(setTableMapOpen, tableMapOpen)} type="button"><Grid3X3 className="owner-shell-study__table-icon" size={14} /><small>ACTIVE TABLE</small><b>{activeTable?.name || 'HOME'}</b><ChevronUp className="owner-shell-study__table-chevron" size={12} /></button>
    ...
    <section aria-label="Central lattice" className="owner-shell-study__canvas" onPointerDown={beginCanvasSelection} ref={canvasRef}>
      <div className="owner-shell-study__table-label"><strong>{activeTable?.name || 'HOME'}</strong><span>TABLE {String(activeTableIndex + 1).padStart(2, '0')} / {activeTable?.public ? 'PUBLIC' : 'PRIVATE'}</span></div>
      {activePlacements.map((placement, index) => {
        ...
        return <Fragment key={placement.id}><div aria-label={`Select ${asset?.title || 'artwork'}`} aria-pressed={selected}
          className="owner-shell-study__placement" data-cropped={Boolean(visibleCrop) || undefined} data-cropping={cropping || undefined} data-frame={presenting ? presentationSession.frame : undefined} data-viewing={viewerPlacementId === placement.id || undefined} key={placement.id}
      ...
      {selectionBounds && !cropSession && !presentationSession && selectedPlacement && <>{selectedPlacements.length > 1 && <div aria-hidden="true" className="owner-shell-study__group-selection" style={{ height: selectionBounds.bottom - selectionBounds.top, left: selectionBounds.left, top: selectionBounds.top, width: selectionBounds.right - selectionBounds.left }} />}{['nw', 'ne', 'se', 'sw'].map((corner) => {
        const east = corner.includes('e');
        const south = corner.includes('s');
        return <button aria-label={`Resize selection from ${corner}`} className={`owner-shell-study__resize-handle is-${corner}`} key={corner}
      ...
      {marquee && <div aria-hidden="true" className="owner-shell-study__marquee" style={marquee} />}
      {drag?.moved && drag.previewRectangle && <div aria-hidden="true" className="owner-shell-study__placement-preview" style={drag.previewRectangle}><img alt="" src={drag.asset.previewSrc} /><span>{drag.asset.title} / RELEASE TO PLACE</span></div>}
    </section>

    {!preview && libraryPresence.present && <section aria-hidden={libraryPresence.phase === 'closing' || undefined} aria-label="Library workspace" className="owner-shell-study__workspace-window owner-shell-study__library owner-shell-study__motion-panel" data-panel-phase={libraryPresence.phase} data-placing={Boolean(drag) || undefined} data-sidebar-collapsed={workspace.sidebarWidth <= 76 || undefined} inert={libraryPresence.phase === 'closing' ? '' : undefined}>
      <Modul8rLibraryAdapter categoryCommands={categoryCommands} data={data}
        onAssetPointerDown={beginAssetDrag} workspace={workspace} />
      <footer className="owner-shell-study__local-rail"><OwnerShellLibraryRail menuSurface={menuSurface} workspace={workspace} /></footer>
    </section>}

    {!preview && !viewerPlacementId && !libraryPresence.present && !settingsPresence.present && !profilePresence.present && !activityPresence.present && !activityHistoryPresence.present && !discoverPresence.present && selectedPlacement && <aside aria-label="Selection and layers inspector" className="owner-shell-study__inspector" data-side={inspectorSide}>
      {cropSession ? <section aria-label="Crop controls" className="owner-shell-study__crop-controls">
      ...
      </section> : presentationSession ? <section aria-label="Frame and mat controls" className="owner-shell-study__presentation-controls">
        <div className="owner-shell-study__presentation-fields">
      ...
      </section> : <><nav aria-label="Selection actions" className="owner-shell-study__selection-actions">
      ...
      <section className="owner-shell-study__layers"><small>LAYERS / THIS TABLE{selectedPlacements.length > 1 ? ` / ${selectedPlacements.length} SELECTED` : ''}</small>
        <div className="owner-shell-study__layer-list">{[...activePlacements].reverse().map((placement) => {
          const asset = ASSETS.find(({ stableAssetId }) => stableAssetId === placement.assetId);
          const confirming = removeCandidateId === placement.id;
          return <div className="owner-shell-study__layer-row" data-confirming={confirming || undefined} data-selected={selectedPlacementIds.includes(placement.id) || undefined} key={placement.id}>
            <button className="owner-shell-study__layer-select" onClick={(event) => { selectPlacement(placement.id, event.shiftKey); setRemoveCandidateId(null); }} type="button"><img alt="" src={asset?.previewSrc} /><span>{asset?.title}</span></button><button aria-label={`Remove ${asset?.title} from table`} className="owner-shell-study__layer-remove" onClick={() => setRemoveCandidateId(placement.id)} title="Remove from table" type="button"><Trash2 size={14} /></button>
            {confirming && <div className="owner-shell-study__remove-confirm"><img alt="" src={asset?.previewSrc} /><span>REMOVE FROM TABLE?</span><button onClick={() => setRemoveCandidateId(null)} type="button">CANCEL</button><button onClick={() => removePlacement(placement.id)} type="button">REMOVE</button></div>}
          </div>;
        })}</div>
      </section>
      </>}
    ...
    {profilePresence.present && !preview && <aside aria-hidden={profilePresence.phase === 'closing' || undefined} aria-label="Profile" className="owner-shell-study__profile owner-shell-study__motion-panel" data-panel-phase={profilePresence.phase} data-viewing={Boolean(identityDossierSession) || undefined} inert={profilePresence.phase === 'closing' ? '' : undefined}>
      <button aria-expanded={Boolean(identityDossierSession)} className="owner-shell-study__profile-card" data-identity-dossier-source="true" data-viewing={Boolean(identityDossierSession) || undefined} onClick={openIdentityDossier} ref={profileIdentityRef} type="button"><div>RV</div><span><b>RADAR VISUALS</b><small>0xPROTOTYPE · OWNER</small></span></button>
    </aside>}

    {identityDossierSession && <OwnerShellIdentityDossier
      getReturnRectangle={() => profileIdentityRef.current?.getBoundingClientRect() || identityDossierSession.originRectangle}
    ...
    {activityPresence.present && !preview && <aside aria-hidden={activityPresence.phase === 'closing' || undefined} aria-label="Activity notifications" className="owner-shell-study__activity-drawer owner-shell-study__motion-panel" data-panel-phase={activityPresence.phase} inert={activityPresence.phase === 'closing' ? '' : undefined}>
      <ol>{ACTIVITY.filter(({ type }) => activityTab === 'ALL' || type === activityTab).map((event) => <li data-unread={event.unread || undefined} key={event.id}><time>{event.time}</time><span><b>{event.label}</b><small>{event.detail}</small></span></li>)}</ol>
      <nav aria-label="Activity filters" className="owner-shell-study__local-rail">{['ALL', 'ASSETS', 'LYX', 'SOCIAL'].map((tab) => <button aria-pressed={activityTab === tab} key={tab} onClick={() => setActivityTab(tab)} type="button">{tab}</button>)}<button aria-label="Open full activity history" onClick={() => { setActivityOpen(false); setActivityHistoryOpen(true); }} title="Open full history" type="button"><History size={13} /></button></nav>
    </aside>}

    {activityHistoryPresence.present && !preview && <section aria-hidden={activityHistoryPresence.phase === 'closing' || undefined} aria-label="Full activity history" className="owner-shell-study__activity-history owner-shell-study__motion-panel" data-panel-phase={activityHistoryPresence.phase} inert={activityHistoryPresence.phase === 'closing' ? '' : undefined}>
      <header>
        <div className="owner-shell-study__activity-history-title"><strong>ACTIVITY HISTORY</strong><span>{visibleActivityHistory.length} EVENTS · 2 UNREAD</span></div>
        <label><span>SEARCH</span><input onChange={(event) => setActivityHistoryQuery(event.target.value)} placeholder="EVENT, ASSET OR PROFILE" type="search" value={activityHistoryQuery} /></label>
        ...
        <button className="owner-shell-study__activity-history-read" onClick={() => showStudyNotice('READ STATE IS SESSION-ONLY IN THIS STUDY')} type="button">MARK ALL READ</button>
        <button aria-label="Close full activity history" onClick={() => setActivityHistoryOpen(false)} type="button"><X size={15} /></button>
      </header>
      {visibleActivityHistory.length > 0 ? <ol>{visibleActivityHistory.map((event) => <li data-unread={event.unread || undefined} key={event.id}>
        <time><strong>{event.date}</strong><small>{event.time}</small></time><i aria-hidden="true" />
      ...
      <footer><span>ACTIVITY RECORD / OWNER SESSION FIXTURE</span><button onClick={() => showStudyNotice('ACTIVITY EXPORT IS NOT CONNECTED IN THIS STUDY')} type="button">EXPORT RECORD</button></footer>
    </section>}

    {discoverPresence.present && !preview && <section aria-hidden={discoverPresence.phase === 'closing' || undefined} aria-label="Discover directory" className="owner-shell-study__workspace-window owner-shell-study__discover owner-shell-study__motion-panel" data-panel-phase={discoverPresence.phase} inert={discoverPresence.phase === 'closing' ? '' : undefined}>
      <div className="owner-shell-study__discover-browser" data-sidebar-collapsed={discoverSidebarWidth <= 118 || viewport.width <= 760 || undefined}
        style={{ '--discover-card-size': `${discoverCardSize}px`, '--discover-sidebar-width': `${discoverSidebarWidth}px` }}>
        <aside aria-label="Discover sections" className="owner-shell-study__discover-sidebar">
          {[
          ...
          <div className="owner-shell-study__discover-group-heading">
        ...
        <button aria-label="Resize Discover sidebar" className="owner-shell-study__discover-sidebar-resize"
          onPointerCancel={finishDiscoverSidebarResize} onPointerDown={beginDiscoverSidebarResize} onPointerMove={updateDiscoverSidebarResize} onPointerUp={finishDiscoverSidebarResize} type="button" />
        {visibleDiscoverPeople.length > 0 ? <div className="owner-shell-study__discover-grid">{visibleDiscoverPeople.map((person) => <button key={person.id} onClick={() => showStudyNotice('PUBLIC PROFILE NAVIGATION IS NOT CONNECTED IN THIS STUDY')} type="button"><img alt="" src={person.asset.previewSrc} />{discoverLabelsVisible && <span><small>{person.role}</small><b>{person.name}</b><em>OPEN PUBLIC PROFILE →</em></span>}</button>)}</div>
          : <p className="owner-shell-study__discover-empty">NO PEOPLE MATCH THIS VIEW.</p>}
      </div>
      <footer className="owner-shell-study__local-rail"><OwnerShellWorkspaceRail clearFilters={() => { setDiscoverQuery(''); setDiscoverFilter('ALL'); }}
    ...
    {settingsPresence.present && !preview && <aside aria-hidden={settingsPresence.phase === 'closing' || undefined} aria-label="Settings" className="owner-shell-study__settings owner-shell-study__motion-panel" data-panel-phase={settingsPresence.phase} inert={settingsPresence.phase === 'closing' ? '' : undefined}>
      <section className="owner-shell-study__settings-section">
        <header><strong>ACTIVITY</strong><span>KEEPER SIGNALS</span></header>
        <div className="owner-shell-study__settings-options">{SIGNAL_OPTIONS.map(([key, label]) => <label key={key}>
      ...
      <section className="owner-shell-study__settings-section owner-shell-study__settings-theme">
        <header><strong>THEME</strong><span>SEPARATE SURFACES</span></header>
        <label><span>WORKSPACE / GRID</span><select onChange={(event) => setGridSurface(event.target.value)} value={gridSurface}>{LATTICE_PRODUCTION_SURFACE_IDS.map((id) => <option key={id} value={id}>{id.toUpperCase()}</option>)}</select></label>
        <label><span>WINDOWS / INTERFACE</span><select onChange={(event) => setMenuSurface(event.target.value)} value={menuSurface}>{LATTICE_PRODUCTION_SURFACE_IDS.map((id) => <option key={id} value={id}>{id.toUpperCase()}</option>)}</select></label>
        <p>SESSION ONLY / NOT PERSISTED</p>
      </section>
      <section className="owner-shell-study__settings-section">
        <header><strong>VISITOR PRESENTATION</strong><span>NEXT PUBLICATION</span></header>
        <div className="owner-shell-study__settings-options">{VISITOR_OPTIONS.map(([key, label]) => <label key={key}>
          <span>{label}</span>
    ...
    {tablePresence.present && !preview && <aside aria-hidden={tablePresence.phase === 'closing' || undefined} aria-label="Table strip" className="owner-shell-study__table-switcher owner-shell-study__motion-panel" data-panel-phase={tablePresence.phase} inert={tablePresence.phase === 'closing' ? '' : undefined}>
      <nav aria-label="Profile tables">{tables.map((table, index) => <button aria-current={table.id === activeTableId ? 'page' : undefined} key={table.id} onClick={() => visitTable(table.id)} type="button"><small>{String(index + 1).padStart(2, '0')}</small><b>{table.name}</b><span>{table.public ? 'PUBLIC' : 'PRIVATE'}</span></button>)}</nav>
      <footer aria-label="Active table controls" className="owner-shell-study__local-rail"><label><span>NAME</span><input maxLength="24" onChange={(event) => updateActiveTable({ name: event.target.value.toUpperCase() })} value={activeTable?.name || ''} /></label><button aria-pressed={activeTable?.public} onClick={() => updateActiveTable({ public: !activeTable?.public })} type="button"><span>VISIBILITY</span><b>{activeTable?.public ? 'PUBLIC' : 'PRIVATE'}</b></button><button aria-label="Add table to the right" onClick={addTable} title="Add table" type="button"><Plus size={13} /></button></footer>
    </aside>}
    ...
    />}
    {notice && <output className="owner-shell-study__notice">{notice}</output>}
    {preview && <div className="owner-shell-study__preview-label">VISITOR PREVIEW / EDITING HIDDEN</div>}
  </main>;
}
