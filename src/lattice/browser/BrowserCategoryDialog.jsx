  ...
  return (
    <div className="lattice-browser-dialog-layer" onPointerDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
      <form aria-labelledby="lattice-browser-dialog-title" aria-modal="true" onSubmit={submit} role="dialog">
        ...
        {deletion ? <p>REMOVE <strong>{dialog.category.name}</strong>? TABLE PLACEMENTS ARE NOT AFFECTED.</p> : <label><span>CATEGORY NAME</span><input maxLength="80" ref={inputRef} value={name} onChange={(event) => setName(event.target.value)} /></label>}
